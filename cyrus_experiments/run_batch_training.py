...
for seed in range(N):
    ...
    exp_config['exp_setup']['seed'] = seed
    exp_name = exp_config['exp_name']
    now = datetime.datetime.now()
    # datetime_exp_name = now.strftime(
    #     "%Y-%m-%d_%H-%M-%S_" + exp_name
    # )
    exp_name = f'{seed}_{exp_name}'

    ex = Experiment(exp_name)
    ex.add_config(exp_config)
    save_path = f'runs/batch_training/{exp_to_train}/' + exp_name
    ...
    def main(
        ...
        print("Starting sacred experiment: {}".format(exp_name))
