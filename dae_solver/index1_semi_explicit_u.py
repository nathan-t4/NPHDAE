...
class DAESolver():
    ...
    def solve_dae(self, z0, u0, T, params, y0_tol=1e-9):
        """
        Solve the DAE
        """
        ...
        y0new, infodict, ier, mesg = fsolve(lambda yy : self.g(x0, yy, u0, T[0], params), y0, full_output=True)
    ...
    def get_consistent_initial_condition(self, z0, u0, t0, params, y0_tol=1e-9):
        x0 = z0[0:self.num_diff_vars]
        y0 = z0[self.num_diff_vars::]

        y0new, infodict, ier, mesg = fsolve(lambda yy : self.g(x0, yy, u0, t0, params), y0, full_output=True)
    ...
    def solve_dae_one_timestep_rk4(self, z0, u0, t0, delta_t, params):
        """
        Solve one forward timestep using RK4.
        Warning that this method does not check for consistency of the algebraic states.
        """
        k1 = self.f_coupled_system(z0, u0, t0, params)
        k2 = self.f_coupled_system(z0 + delta_t/2 * k1, u0, t0 + delta_t/2, params)
        k3 = self.f_coupled_system(z0 + delta_t/2 * k2, u0, t0 + delta_t/2, params)
        k4 = self.f_coupled_system(z0 + delta_t * k3, u0, t0 + delta_t, params)
