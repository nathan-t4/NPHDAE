...
import sys
sys.path.append('../')
from dae_solver.index1_semi_explicit_random import RandomDAESolver
...
class RandomPHDAE():
    ...
    def construct_f_and_g(self):
        ...
        def f(x, y, t, jax_key, params):
            ...
            E, J, z_vec, diss, B = self.construct_matrix_equations(x, y, t, jax_key,params) 

            rhs = jnp.linalg.matmul(J, z_vec) - diss + jnp.linalg.matmul(B, self.u_func(t, jax_key, u_func_params))
    ...
    def construct_matrix_equations(self, x, y, t, jax_key, params):
        ...
        curr_through_resistors = jnp.linalg.matmul(self.AR, self.r_func(jnp.linalg.matmul(self.AR.transpose(), e), jax_key, r_func_params)) # The current through the resistors.
        diss = diss.at[0:self.num_nodes].set(curr_through_resistors)

        charge_constraint = self.q_func(jnp.linalg.matmul(self.AC.transpose(), e), jax_key, q_func_params) - q
        ...
        # Construct z_vec
        z_vec = jnp.concatenate((e, self.grad_H_func(phi, jax_key, grad_H_func_params), q, jv))
    
        return E, J, z_vec, diss, B

    def construct_dae_solver(self):
        self.solver = RandomDAESolver(self.f, self.g, self.num_differential_vars, self.num_algebraic_vars)

    def solve(self, z0, T, jax_key, params, tol=1e-6):
        return self.solver.solve_dae(z0, T, jax_key, params, tol)
...
if __name__ == "__main__":
    ...
    dae = RandomPHDAE(AC, AR, AL, AV, AI, grad_H_func=grad_H_func, q_func=q_func, r_func=r_func, u_func=u_func)
    
    key = jax.random.split(0)
    sol = dae.solve(z0, T, key, params=None)
